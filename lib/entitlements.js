...
const FEATURES = {
    ...
    // Agency Reseller V2 (Release B): the right to sell client deliverables,
    // kept separate from COMMERCIAL_LICENSE (the right to use outputs
    // commercially at all) so either can be sold independently later.
...
async function seedPlanEntitlements(userId, planId, planConfig) {
    ...
    if (planConfig.agency_license) wanted.push(FEATURES.AGENCY_LICENSE);
    // Granted honestly rather than left off the bundle, but there is no
    // queueing mechanism yet to act on it -- do not market this as "faster"
    // until one exists.
    if (planConfig.priority) wanted.push(FEATURES.PRIORITY_QUEUE);
