...
// Without Upstash the limiters are mocked to always pass, which is fine locally
// but means the paid AI endpoints are completely unthrottled. That used to
// happen silently; in production it is now announced loudly, and can be made a
// hard failure with RATELIMIT_REQUIRED=true.
const hasRedis = !!(process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN);

if (!hasRedis) {
    const message =
        'Rate limiting is DISABLED — UPSTASH_REDIS_REST_URL / UPSTASH_REDIS_REST_TOKEN are not set. ' +
        'Generation endpoints that cost money are unthrottled.';
    if (process.env.RATELIMIT_REQUIRED === 'true') {
