...
class PolicyEngine {
    ...
    static async evaluateRequest(context, estimatedCosts) {
        ...
        try {
            ...
            // 3. Workspace Active
            // Previously commented out with no status column to read, so a
            // suspended workspace carried on generating. Enforcement has to sit
            // here rather than in a route, or it only covers whichever routes
            // remembered to check.
            const workspaceStatus = await db.getWorkspaceStatus(context.userId);
            if (workspaceStatus && workspaceStatus !== 'active') {
                reasons.push(`Workspace is ${workspaceStatus}.`);
            }
