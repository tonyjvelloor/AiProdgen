...
async function getPartnerSummary(partnerId) {
    ...
    // Keyed per currency -- MIN_PAYOUT_CENTS is already currency-specific
    // (2500 cents means something completely different for USD vs INR), so
    // summing commission_amount across currencies into one number would
    // silently misrepresent a partner's balance the moment any commission
    // isn't USD. The $47 offer already accepts INR (LIFETIME_OFFER_PRICES),
    // so this is not a hypothetical: a single INR referral would have
    // inflated a USD-denominated total by roughly 80x.
    const now = Date.now();
    const byCurrency = {}; // { USD: { pendingCents, availableCents, paidCents }, ... }
    const bucket = (currency) => (byCurrency[currency] || (byCurrency[currency] = { pendingCents: 0, availableCents: 0, paidCents: 0 }));
    const toApprove = [];
    for (const row of rows || []) {
        const b = bucket(row.currency || 'USD');
        if (row.status === 'pending' && new Date(row.eligible_at).getTime() <= now) {
            toApprove.push(row.id);
            b.availableCents += row.commission_amount;
        } else if (row.status === 'pending') {
            b.pendingCents += row.commission_amount;
        } else if (row.status === 'available') {
            b.availableCents += row.commission_amount;
        } else if (row.status === 'paid') {
            b.paidCents += row.commission_amount;
        }
    }
    if (toApprove.length) {
    ...
    for (const currency of Object.keys(byCurrency)) {
        ...
        b.totalEarnedCents = b.pendingCents + b.availableCents + b.paidCents;
        b.payoutEligible = b.availableCents >= (MIN_PAYOUT_CENTS[currency] ?? MIN_PAYOUT_CENTS.USD);
    }

    // USD convenience fields at the top level -- USD is what every current
    // SKU actually charges (checkout forces useCurrency = 'USD' regardless
    // of display currency; only the older $47 order-creation path can carry
    // INR), so this keeps the common case a flat read while byCurrency
    // carries the full, correct picture for anything else.
    const usd = byCurrency.USD || { pendingCents: 0, availableCents: 0, paidCents: 0, totalEarnedCents: 0, payoutEligible: false };

    return {
        pendingCents: usd.pendingCents,
        availableCents: usd.availableCents,
        paidCents: usd.paidCents,
        totalEarnedCents: usd.totalEarnedCents,
        payoutEligible: usd.payoutEligible,
        byCurrency,
        referralCounts
    };
}
